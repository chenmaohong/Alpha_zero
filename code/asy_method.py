from train import TrainPipeline
from policy_value_net_pytorch import PolicyValueNet, Net
import copy
from multiprocessing import Pool, cpu_count, set_start_method
import torch.optim as optim
import torch
import torch.multiprocessing as mp
import time

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')


class worker(TrainPipeline):
    def __init__(self, global_net):
        super(worker, self).__init__(global_net)
        self.policy_value_net.policy_value_net = copy.deepcopy(global_net)

    def task(self, global_net, index, optimizer):
        for i in range(100):
            self.policy_value_net.policy_value_net = copy.deepcopy(global_net)
            loss = self.asy_run()
            loss.backward()
            optimizer.step()
            # if i == 10 * index:
            #     net_params = global_net.state_dict()
            #     model_file = './current_policy_8.model'
            #     torch.save(net_params, model_file, _use_new_zipfile_serialization=False)
            print("Index:{}, batch:{}, loss:{:.4f}".format(index, i + 1, loss.data))


class asy_train(object):
    def __init__(self, num_agent, board_width, board_height, model_file='current_policy.model'):
        self.num = num_agent
        self.round = 1000
        self.l2_const = 1e-4
        self.global_net = Net(board_width, board_height).to(device)
        self.optimizer = optim.Adam(self.global_net.parameters(), weight_decay=self.l2_const)
        # if model_file:
        #     net_params = torch.load(model_file)
        #     self.global_net.load_state_dict(net_params)
        self.agent_set = list()
        for i in range(num_agent):
            self.agent_set.append(worker(self.global_net))

    def run(self):
        set_start_method('spawn')
        p = Pool(processes=self.num)
        for i in range(self.num):
            p.apply_async(self.task, args=(i,))
        p.close()
        p.join()

    def task(self, i):
        for k in range(100):
            self.agent_set[i].policy_value_net.policy_value_net = copy.deepcopy(self.global_net)
            loss = self.agent_set[i].asy_run()
            print("Index:{}, batch:{}, loss:{:.4f}".format(i, k + 1, loss.data))
            # self.optimizer.zero_grad()
            # loss.backward()
            # self.optimizer.step()



def task(i, global_net, optimizer):
    agent = worker(global_net)
    loss = agent.asy_run()
    loss.backward()
    optimizer.step()
    print("batch:{}, loss:{:.4f}".format(i + 1, loss.data))


if __name__ == '__main__':
    start = time.time()
    # board_width, board_height = 8, 8
    # l2_const = 1e-4
    # global_net = Net(board_width, board_height).to(device)
    # model_file = 'current_policy_15.model'
    # net_params = torch.load(model_file)
    # global_net.load_state_dict(net_params)
    # optimizer = optim.Adam(global_net.parameters(), weight_decay=l2_const)
    # p = Pool(processes=cpu_count())
    # for i in range(1000):
    #     # if (i + 1) % 50 == 0:
    #     #     net_params = global_net.state_dict()
    #     #     # torch.save(net_params, model_file)
    #     #     model_file = './current_policy_15.model'
    #     #     torch.save(net_params, model_file, _use_new_zipfile_serialization=False)
    #     p.apply_async(task, args=(i, global_net, optimizer))
    # p.close()
    # p.join()

    asy_Pipe = asy_train(cpu_count()-1, 8, 8)
    asy_Pipe.run()
    end = time.time()
    print("The training time is {}".format(end - start))
